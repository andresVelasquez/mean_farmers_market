...
function findMarkets(zip) {
    ...
    clearMap();
    $.get("http://search.ams.usda.gov/farmersmarkets/v1/data.svc/zipSearch?zip=" + zip, function(response){
        var marketArr = [];
        for(var i = 0; i < response["results"].length; i++) {
            marketArr.push(response["results"][i]["id"]);
        }
        $.post("/filtermarkets", {"markets": marketArr}).then(function(response){
          filteredMarkets = response;
          saveData();
        })
        // $.get("/marketdata", function(response){saveData(response, marketArr);}, "json");
    })
}
...
function goToWall() {
    window.location.href = "/markets/" + filteredMarkets[activeMarketIndex].FMID;
}
